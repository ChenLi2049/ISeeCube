...
def prepare_sensors(path):
    ...
    sensors["string"] = 0
    sensors["qe"] = 0  # 1 #Quantum Efficiency
...
def ice_transparency(path, datum=1950):
    ...
    # Taken at the center of IceCube (depth of 1950 m, temperature −30.4 ◦C); cf. ρ = 0.9167 at 1 atm. at 0 ◦ C.
...
class IceCubeCache(Dataset):
    ...
    def __getitem__(self, idx0):
        ...
        sensor_id = df["sensor_id"].item().to_numpy()
        if self.mask_only:
            mask = torch.ones(min(len(sensor_id), self.L), dtype=torch.long)
            return {"mask": mask}, {}
        time = df["time"].item().to_numpy()
        charge = df["charge"].item().to_numpy()
        auxiliary = df["auxiliary"].item().to_numpy()
